...
# Note the use of t1 size instead of t0; model is thus slightly different than Drees and Shea (2023)
...
plot.span <- function(x, y, left = FALSE, bottom = FALSE, type = "real"){
  ...
  if(left == FALSE & bottom == FALSE){
    plot(x, y, xaxt = "n", yaxt = "n", xlab = "", ylab = "", ylim = c(0, 50),
         cex = 0.5, col = colour, pch = 16)
    axis(side = 1, at = x, labels = FALSE)
    axis(side = 2, at = seq(0, 50, by = 10), labels = FALSE)}
  if(left == TRUE & bottom == FALSE){
    plot(x, y, xaxt = "n", yaxt = "n", xlab = "", ylab = "Wavespeed (m/yr)", ylim = c(0, 50),
         cex = 0.5, col = colour, pch = 16)
    axis(side = 1, at = x, labels = FALSE)
...
# Define function to generate GIF of population spread (nYear = 100 recommended)
